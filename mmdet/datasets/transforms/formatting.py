...
class PackDetInputs(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
            ...
            if key == 'gt_masks' or isinstance(results[key], BaseBoxes):
                ...
                if 'gt_ignore_flags' in results:
                    ...
                    instance_data[self.mapping_table[key]] = to_tensor(
                        results[key])
        ##### by cpf
        if 'gt_valid_ids' in results:
            instance_data['gt_valid_ids'] = to_tensor(results['gt_valid_ids'][valid_idx])
        ######
        ...
        if 'proposals_boxes' in results:
            proposals['bboxes'] = results['proposals_boxes']

        if 'proposals_segms' in results:
            proposals['masks'] = results['proposals_segms']
        if 'proposals_segm_mode' in results:
            proposals['proposals_segm_mode'] = [results['proposals_segm_mode'] for i in results['proposals_boxes']]
        if 'roi_segms' in results:
            proposals['roi_segms'] = results['roi_segms']
        if 'proposals_valid_ids' in results:
            proposals['proposals_valid_ids'] = to_tensor(results['proposals_valid_ids'])
        data_sample.proposals = proposals
