...
class Resize(MMCV_Resize):
    ...
    def _resize_proposal_masks(self, results: dict) -> None:
        """Resize masks with ``results['scale']``"""
        if results.get('proposals_segms', None) is not None:
            if self.keep_ratio:
                results['proposals_segms'] = results['proposals_segms'].rescale(
                    results['scale'])
            else:
                results['proposals_segms'] = results['proposals_segms'].resize(
                    results['img_shape'])

    def _resize_proposal_bboxes(self, results: dict) -> None:
        """Resize bounding boxes with ``results['scale_factor']``."""
        if results.get('proposals_boxes', None) is not None:
            results['proposals_boxes'].rescale_(results['scale_factor'])
            if self.clip_object_border:
                results['proposals_boxes'].clip_(results['img_shape'])
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'proposals_boxes' in results:
            self._resize_proposal_bboxes(results)
        if 'proposals_segms' in results and results['proposals_segm_mode'] == 'AugImage':
            self._resize_proposal_masks(results)
...
class RandomFlip(MMCV_RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        if results.get('proposals_boxes', None) is not None:
            results['proposals_boxes'].flip_(img_shape, results['flip_direction'])

        if results.get('proposals_segms', None) is not None and results['proposals_segm_mode'] == 'AugImage':
            results['proposals_segms'] = results['proposals_segms'].flip(
                results['flip_direction'])

        if results.get('roi_segms', None) is not None:
            results['roi_segms'] = results['roi_segms'].flip(
                results['flip_direction'])
...
class RandomCrop(BaseTransform):
    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
        ...
        if results.get('gt_bboxes', None) is not None:
            ...
            results['gt_valid_ids']=valid_inds.nonzero()[0]
        ...
        if results.get('proposals_boxes', None) is not None:
            bboxes = results['proposals_boxes']
            bboxes.translate_([-offset_w, -offset_h])
            import torch
            ul = -(bboxes.tensor[:, :2].clamp(-1e8, 0) / (bboxes.tensor[:, 2:] - bboxes.tensor[:, :2]))
            dr = 1 - (bboxes.tensor[:, 2:] - torch.tensor([img_shape[1], img_shape[0]])).clamp(0) / (
                    bboxes.tensor[:, 2:] - bboxes.tensor[:, :2])
            roi_crop_ratio = torch.cat([ul, dr], dim=1)

            if self.bbox_clip_border:
                bboxes.clip_(img_shape[:2])

            valid_pps_inds = bboxes.is_inside(img_shape[:2]).numpy()
            # If the crop does not contain any gt-bbox area and
            # allow_negative_crop is False, skip this image.
            if (not valid_pps_inds.any() and not allow_negative_crop):
                return None

            valid_pps_inds *= results['roi_segms'].masks.reshape(-1, 49).max(1)
            results['roi_segms'] = results['roi_segms'][valid_pps_inds.nonzero()[0]].crop_and_resize(
                roi_crop_ratio[valid_pps_inds] * 7,
                (7, 7),
                np.arange(len(roi_crop_ratio[valid_pps_inds])))

            valid_pps_inds[valid_pps_inds] *= results['roi_segms'].masks.reshape(-1, 49).max(1)
            results['roi_segms']=results['roi_segms'][valid_pps_inds[valid_pps_inds].nonzero()[0]]
            results['proposals_boxes'] = bboxes[valid_pps_inds]
            if results.get('proposals_segms', None) is not None and results['proposals_segm_mode'] == 'AugImage':
                results['proposals_segms'] = results['proposals_segms'][
                    valid_pps_inds.nonzero()[0]].crop(
                    np.asarray([crop_x1, crop_y1, crop_x2, crop_y2]))
                if self.recompute_bbox:
                    results['proposals_boxes'] = results['proposals_segms'].get_bboxes(
                        type(results['proposals_boxes']))
            results['proposals_valid_ids'] = valid_pps_inds.nonzero()[0]
